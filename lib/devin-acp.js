...
import { DEVIN_PROVIDER, DEVIN_EFFORT_SUFFIX, devinApiKey, devinCachedModelSlugs } from './devin-models.js'
...
// api_key 由 devin-models.js 的 devinApiKey() 从 credentials.toml 或
// DEVIN_API_KEY / WINDSURF_API_KEY 环境变量解析。没有 key 时仍尝试
// session/new——旧版本可能允许本地凭据直通;失败再报"未登录"。
...
export async function startDevinAcpRun(deps, request) {
  ...
  // 其它 provider 的模型对 Devin 无意义,不透传。推理档位编码在 slug 里:
  // 基础模型 + reasoningEffort → <model>-<effort>(模型已带档位后缀则不拼)。
  const argv = [executable, 'acp']
  if (request.provider === DEVIN_PROVIDER && typeof request.model === 'string' && request.model) {
    const effort = typeof request.reasoningEffort === 'string' && request.reasoningEffort
    let slug = request.model
    if (effort && !DEVIN_EFFORT_SUFFIX.test(request.model)) {
      slug = `${request.model}-${effort}`
      // 各厂商基础项写法不同(gpt-5.6-luna 带点),变体统一用连字符
      // (gpt-5-6-luna-high)。拼接结果不在账号目录时改试归一化候选。
      const alt = `${request.model.replace(/[._]/g, '-')}-${effort}`
      const cached = devinCachedModelSlugs(deps)
      if (alt !== slug && !cached.has(slug) && cached.has(alt)) slug = alt
    }
    argv.push('--model', slug)
  }
      ...
      settle({
        ...
        ...(signal.aborted || disposing ? {} : {
          diagnostic: authFailure
            // 保留原始错误:上游 403/限流/模型侧鉴权失败也会命中 AUTH_FAILURE,
            // 吞掉原文会让真实原因不可诊断(例如误判为本地凭据失效)。
            ? `Devin CLI 未登录或凭据失效：请先运行 devin auth login（或设置 DEVIN_API_KEY）· 原始错误：${message}`
