    ...
    accept(event, state, nativeSession) {
      ...
      if (event.type === 'result') {
        ...
        if (event.is_error || (event.subtype && event.subtype !== 'success')) {
          ...
          // is_error 时常态携带真实原因(如 "Not logged in"),subtype 只是
          // 类别标签——诊断必须带上原文,否则只剩误导性的 "exit 1:success"。
          if (event.is_error && typeof event.result === 'string' && event.result) state.resultError = event.result
          else if (event.is_error && Array.isArray(event.errors)) {
            const detail = event.errors.find((entry) => typeof entry === 'string' && entry)
            if (detail) state.resultError = detail
          }
        }
      }
    },
...
async function startProcessRun(deps, harness, request) {
  ...
  const result = (async () => {
    ...
    if (state.failed || attempt.outcome?.exitCode !== 0) {
      const detail = state.resultError ?? (state.resultSubtype && state.resultSubtype !== 'success' ? state.resultSubtype : undefined)
      return {
        output: state.text ? [{ type: 'text', text: state.text }] : [],
        stopReason: 'error',
        diagnostic: `${harness} 执行失败（exit ${String(attempt.outcome?.exitCode)}${detail ? `：${detail}` : ''}）`,
      }
    }
