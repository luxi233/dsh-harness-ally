...
import { CLAUDE_OWN_PROVIDER } from './cli-own-models.js'
...
const ADAPTERS = Object.freeze({
  'claude-code': {
    ...
    command: 'claude',
    argv(executable, model, bridge, nativeSession, hasImages) {
      return [
        executable,
        '-p',
        ...(bridge ? [
...
async function startProcessRun(deps, harness, request) {
  ...
  if (resolvedFiles.length) initialPrompt = filePathFallback(initialPrompt, resolvedFiles)
  ...
  const spawnAttempt = () => {
    const nativeArgv = adapter.argv(executable, ownConfig ? undefined : request.model, bridgeRoute, request.nativeSession, resolvedImages.length > 0)
    let argv = nativeArgv
