...
async function startProcessRun(deps, harness, request) {
  ...
  const spawnAttempt = () => {
    const nativeArgv = adapter.argv(executable, ownConfig && request.model === CLI_CONFIG_MODEL_ID ? undefined : request.model, bridgeRoute, request.nativeSession, resolvedImages.length > 0)
    let argv = nativeArgv
