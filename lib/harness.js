...
async function startProcessRun(deps, harness, request) {
  ...
  const spawnAttempt = () => {
    ...
    // 启动 .cmd 会抛 spawn EINVAL。claude.cmd shim 实际调 <basedir>/node_modules/@anthropic-ai/claude-code/bin/claude.exe,
    // codex.cmd 调 node + codex.js。统一在这里把 argv[0] 替换成真实可执行文件。
    if (process.platform === 'win32' && typeof executable === 'string' && executable.toLowerCase().endsWith('.cmd')) {
      const basedir = dirname(executable)
      const RESOLVED_NATIVE = {
        'claude-code': join(basedir, 'node_modules', '@anthropic-ai', 'claude-code', 'bin', 'claude.exe'),
        codex: join(basedir, 'node_modules', '@openai', 'codex', 'bin', 'codex.js'),
        'kimi-code': join(basedir, 'node_modules', '@moonshot-ai', 'kimi-code', 'bin', 'kimi.js'),
      }
      const target = RESOLVED_NATIVE[harness]
      if (target && target.endsWith('.js')) {
        argv = [process.execPath, target, ...nativeArgv.slice(1)]
      } else if (target) {
        argv = [target, ...nativeArgv.slice(1)]
      }
    }
