...
async function listModelsViaAcp(deps) {
  ...
  try {
    ...
    return modelsFromConfigOptions(created)
  } finally {
  ...
  const listAccountModels = async () => {
    ...
    catalog = (async () => {
      try {
        ...
        // models list 不可用(未登录判定 bug 或旧版本无此命令)时走 ACP 握手。
        const viaAcp = await listModelsViaAcp(deps).catch(() => [])
        return viaAcp.length ? viaAcp : FALLBACK_MODELS.map((m) => ({ provider: DEVIN_PROVIDER, ...m }))
      } catch {
        const viaAcp = await listModelsViaAcp(deps).catch(() => [])
        return viaAcp.length ? viaAcp : FALLBACK_MODELS.map((m) => ({ provider: DEVIN_PROVIDER, ...m }))
      }
