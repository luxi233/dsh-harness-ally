...
// 缓存 bin 是 protobuf:顶层 repeated field-1 子消息,每条一个模型记录;
// 记录内 field 18 是该模型的 context window varint(swe-2 系=262000,
// gpt-5-6-luna 系=1000000)。只做最小字段解析,拿不到就退回无窗口。
...
function protobufFields(buf) {
  ...
  while (offset < buf.length) {
    ...
    if (wire === 0) {
      const data = readVarint(buf, offset)
      if (!data) break
      offset += data.length
      out.push({ field, wire, value: data.value })
    } else if (wire === 2) {
      const len = readVarint(buf, offset)
      if (!len) break
      offset += len.length
      out.push({ field, wire, buf: buf.subarray(offset, offset + len.value) })
      offset += len.value
...
const WINDOW_FIELD = 18
const isWindow = (value) => Number.isSafeInteger(value) && value >= 8192 && value <= 16 * 1024 * 1024
function recordContextWindow(buf) {
  const fields = protobufFields(buf)
  const slug = fields
    .filter((f) => f.wire === 2)
    .map((f) => { try { return f.buf.toString('utf8') } catch { return '' } })
    .find((s) => MODEL_CACHE_PREFIXES.test(s) && /[0-9.-]/.test(s))
  const window = fields.find((f) => f.wire === 0 && f.field === WINDOW_FIELD)?.value
  if (!slug || !isWindow(window)) return undefined
  return { slug, contextWindow: window }
}
...
function modelCatalogFromCache(deps) {
  ...
  for (const dir of dirs) {
    ...
    for (const name of files) {
      ...
      for (const record of protobufFields(buf)) {
        if (record.wire !== 2 || record.buf.length < 50) continue
        const parsed = recordContextWindow(record.buf)
        if (parsed && !windows.has(parsed.slug)) windows.set(parsed.slug, parsed.contextWindow)
      }
      let current = ''
      const strings = []
      ...
      // 缓存里每个模型按 effort/tier 展开成多个变体条目,全部保留——
      // 用户要的是完整档位列表(swe-2/-medium/-high/-max 都可选)。
      for (const token of strings) {
        const slug = token.trim().toLowerCase()
        if (!MODEL_CACHE_PREFIXES.test(slug) || !/[0-9.-]/.test(slug)) continue
        if (found.some((model) => model.id === slug)) continue
        found.push({
          provider: DEVIN_PROVIDER,
          id: slug,
          name: slug,
          ...windows.has(slug) ? { contextWindow: windows.get(slug) } : {},
        })
      }
    }
  }
...
// 合并所有目录源取并集:models list(官方基础列表)、ACP configOptions
// (官方档位列表,会漏部分 slug 如无档位的 swe-2 基础项)、model_configs
// bin 缓存(账号级 slug 全集)。去重保持先到优先,再补 ACP currentValue,
// 最后给有档位变体的基础 slug 挂 supportedReasoningEfforts。
async function mergedAccountCatalog(deps, base = []) {
  const acp = await listModelsViaAcp(deps).catch(() => ({ models: [] }))
  const merged = []
  for (const entry of [...base, ...(acp.models ?? []), ...modelCatalogFromCache(deps)]) {
    if (!entry?.id) continue
    const existing = merged.find((model) => model.id === entry.id)
    // 去重先到优先,但 contextWindow 只有缓存条目带——被丢弃的重复
    // 缓存条目要把窗口补到已保留的官方条目上。
    if (existing) {
      if (existing.contextWindow === undefined && isWindow(entry.contextWindow)) {
  ...
  if (acp.current && !merged.some((model) => model.id === acp.current)) {
    merged.unshift({ provider: DEVIN_PROVIDER, id: acp.current, name: acp.current })
  }
  // context window 只在 bin 缓存记录里有:基础 slug(swe-2、gpt-5.6-luna)
  // 大多没有自己的记录,从家族变体派生——逐级剥掉日期/通道/档位后缀
  // 注册中间 key,归一化后基础项直接命中。
  const windows = new Map()
  for (const model of merged) {
    if (!isWindow(model.contextWindow)) continue
    let key = normalizeSlug(model.id)
    while (key) {
      if (!windows.has(key)) windows.set(key, model.contextWindow)
      const stripped = stripVariantSuffix(key)
  ...
  for (const model of merged) {
    ...
    while (key) {
      ...
      if (window !== undefined) { model.contextWindow = window; break }
      const stripped = stripVariantSuffix(key)
      if (stripped === key) break
      key = stripped
    }
  }
  if (merged.length) return attachReasoningEfforts(merged, acp.current)
  return FALLBACK_MODELS.map((m) => ({ provider: DEVIN_PROVIDER, ...m }))
}

// Devin 把推理档位编码在模型 slug 里(swe-2-high、claude-opus-5-max…)。
// 给有变体的基础 slug 挂 supportedReasoningEfforts,选择器出档位下拉;
// 派发时 devin-acp 把 model+effort 拼回 <model>-<effort>。
export const DEVIN_EFFORT_ORDER = ['minimal', 'none', 'low', 'medium', 'high', 'xhigh', 'max']
export const DEVIN_EFFORT_SUFFIX = /-(minimal|none|low|medium|high|xhigh|max)$/
// 各厂商命名约定不同:基础项可能是 gpt-5.6-luna(OpenAI 版本号带点)
// 或 swe-1.7,而档位变体是 gpt-5-6-luna-high、swe-1-7-medium。
// 家族归组时把 . _ - 分隔符视为等价,不按字符串精确匹配。
const normalizeSlug = (id) => id.replace(/[._]/g, '-')
// 变体后缀剥链:日期版(-09102026)、速度通道(-priority/-fast)、推理档位
// (-high 等)。逐级剥用于把变体的 contextWindow 归到基础 slug 上。
const stripVariantSuffix = (id) => id
  .replace(/-\d{8}$/, '')
  .replace(/-(?:priority|fast)$/, '')
  .replace(DEVIN_EFFORT_SUFFIX, '')
function attachReasoningEfforts(models, current) {
  const byNorm = new Map()
  for (const model of models) {
    const norm = normalizeSlug(model.id)
    if (!byNorm.has(norm)) byNorm.set(norm, [])
    byNorm.get(norm).push(model)
  }
  const effortsByBase = new Map()
  for (const model of models) {
    const match = DEVIN_EFFORT_SUFFIX.exec(model.id)
    if (!match) continue
    const baseNorm = normalizeSlug(model.id.slice(0, -match[0].length))
    for (const base of byNorm.get(baseNorm) ?? []) {
      if (base.id === model.id) continue
      if (!effortsByBase.has(base.id)) effortsByBase.set(base.id, new Set())
      effortsByBase.get(base.id).add(match[1])
    }
  }
  const currentEffort = typeof current === 'string' ? DEVIN_EFFORT_SUFFIX.exec(current)?.[1] : undefined
  for (const model of models) {
    const efforts = effortsByBase.get(model.id)
    if (!efforts?.size) continue
    const ordered = DEVIN_EFFORT_ORDER.filter((effort) => efforts.has(effort))
    model.supportedReasoningEfforts = ordered.map((effort) => ({ reasoningEffort: effort, name: effort }))
    const preferred = typeof current === 'string' && normalizeSlug(current).startsWith(`${normalizeSlug(model.id)}-`) ? currentEffort : undefined
    model.defaultReasoningEffort = preferred ?? (efforts.has('medium') ? 'medium' : ordered[0])
  }
  return models
}

// 账号目录 slug 全集(含档位变体),供 devin-acp 把 model+effort
// 翻译成真实存在的 slug(官方目录点/连字符写法混用)。
export function devinCachedModelSlugs(deps) {
  return new Set(modelCatalogFromCache(deps).map((model) => model.id))
}

// 与 cli-own-models 的 reasoningMetadata 同形:resolveModel 结果里带
// reasoning.efforts,DSH 选择器据此渲染档位下拉。
  ...
  const listAccountModels = async () => {
    ...
    catalog = (async () => {
      try {
        ...
        return await mergedAccountCatalog(deps, found)
      } catch {
        return await mergedAccountCatalog(deps)
      }
    })()
  ...
  return {
    ...
    async resolveModel(provider, model) {
      ...
      return {
        ...
        ...reasoningMetadata(entry),
        ...isWindow(entry?.contextWindow) ? { context: { contextWindow: entry.contextWindow } } : {},
      }
    },
