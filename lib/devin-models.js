...
function attachReasoningEfforts(models, current) {
  ...
  for (const model of models) {
    ...
    if (!effortsByBase.has(base)) effortsByBase.set(base, new Set())
    effortsByBase.get(base).add(match[1])
  }
  ...
  for (const model of models) {
    ...
    const preferred = typeof current === 'string' && current.startsWith(`${model.id}-`) ? currentEffort : undefined
