...
export async function apply(ctx) {
  ...
  const gateway = createHarnessGateway({
    ...
    nativeSessions,
    stateDir: state.dir,
    // 惰性解析:attachments 服务在老版本 DSH 上可能缺席,不进 inject 声明,
    // 避免整个插件因缺服务而永远不 apply;图片请求时再取,缺失则友好报错。
    get attachments() { return ctx.attachments },
  })
  ...
  try {
    ctx.llm.registerAdapter([DEVIN_PROVIDER], createDevinModelAdapter({ subprocess: ctx.subprocess, cliManager }))
  } catch (error) {
    ctx.logger.warn(`ally devin provider registration failed: ${error instanceof Error ? error.message : String(error)}`)
  }
  for (const ownAdapter of ownConfigAdapters()) {
    try {
