...
// ⚠️ 加进这里意味着任何能访问该 host 并通过 dsh-bridge 认证的人
// 都能调 /ally/select 与 /ally/cli-install。
const ALLOWED_REMOTE_HOSTS = new Set([
  '113.31.118.243:3002', // dsh-bridge customTunnel 公网入口
])

// 诊断日志：写入固定路径,方便 grep;不影响功能
import { appendFileSync } from 'node:fs'
import { tmpdir } from 'node:os'
import { join } from 'node:path'
const DEBUG_LOG = join(tmpdir(), 'dsh-ally-debug.log')
function debugLog(stage, payload) {
  try {
    appendFileSync(DEBUG_LOG, `[${new Date().toISOString()}] ${stage} ${JSON.stringify(payload)}\n`)
  } catch {}
}
...
function trustedRead(req) {
  ...
  debugLog('trustedRead', { host, origin, secFetchSite, authorityHost: authority?.host })
  if (!authority) return false
...
export function trustedMutation(req) {
  ...
  const origin = req.headers?.origin
  debugLog('trustedMutation', { secFetchSite, contentType, origin, trustedRead: trustedRead(req) })
...
export async function apply(ctx) {
  ...
  const routes = {
    ...
    'POST /ally/cli-install': async (req, res) => {
      ...
      // 失败只记 debugLog。客户端 UI 显示"安装中",完成后由 BroadcastChannel 或
      // 下一次 status 轮询触发 UI 更新。
      cliManager.install(harness).catch((error) => {
        debugLog('cliInstallFail', { harness, error: error instanceof Error ? error.message : String(error) })
      })
      sendJson(res, 200, { harnesses: await cliManager.status() })
    },
  }
