...
export const KIMI_OWN_PROVIDER = 'kimi-code'
...
const CLAUDE_MODELS = [
  { id: 'sonnet', name: 'Claude Sonnet', description: 'Claude Code default balanced model', contextWindow: 200000 },
  { id: 'opus', name: 'Claude Opus', description: 'Claude Code high-capability model', contextWindow: 200000 },
  { id: 'haiku', name: 'Claude Haiku', description: 'Claude Code fast model', contextWindow: 200000 },
]

// resolveModel 结果里带 context.contextWindow 时,DSH 会写进 request/context
// 事件,输入框下的上下文占用圆环据此渲染分母;没有数据的 provider 保持缺省,
// 圆环不渲染而不是瞎猜一个上限。
function contextMetadata(row) {
  const window = row?.contextWindow ?? row?.context_window
  return Number.isSafeInteger(window) && window > 0 ? { context: { contextWindow: window } } : {}
}
...
async function codexAccountModels(deps) {
  ...
  const models = rows
    ...
    .map((row) => ({
      ...
      ...Number.isSafeInteger(row.contextWindow ?? row.context_window) && (row.contextWindow ?? row.context_window) > 0
...
// Kimi Code:`kimi acp` 握手 initialize → session/new,configOptions 里
// category==='model' 的 select 携带账号模型目录;category==='thought_level'
// 的 select 是推理档位(low/high/max)。未登录时 session/new 返回
// Authentication required,回退为「跟随 CLI 配置」占位。
async function kimiAccountModels(deps) {
  const executable = deps.cliManager
    ? await deps.cliManager.resolve(KIMI_OWN_PROVIDER)
    : await deps.subprocess.resolveExecutable('kimi')
  const child = deps.subprocess.spawn({
    argv: [executable, 'acp'],
    cwd: deps.cwd ?? homedir(),
    stdio: { stdin: 'pipe', stdout: 'pipe', stderr: { maxBytes: 64 * 1024 } },
  ...
  try {
    created = await new Promise((resolve, reject) => {
      const decoder = new StringDecoder('utf8')
      let buffer = ''
      const timeout = setTimeout(() => reject(new Error('kimi session/new timeout')), 10000)
      const finish = (value, error) => {
        clearTimeout(timeout)
      ...
      child.stdout?.on('data', (chunk) => {
        ...
        while ((newline = buffer.indexOf('\n')) >= 0) {
          ...
          if (hasId && pending.has(message.id) && typeof message.method !== 'string') {
            ...
            // 目录探测不处理 server 请求(权限等),直接回 method-not-found。
            child.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id: message.id, error: { code: -32601, message: 'Unsupported client method' } })}\n`)
          }
        }
      })
      child.done.then(() => finish(undefined, new Error('kimi ACP exited early')), () => {})
      send('initialize', {
        protocolVersion: 1,
        clientInfo: { name: 'dsh-ally', version: ALLY_VERSION },
        clientCapabilities: { fs: { readTextFile: false, writeTextFile: false } },
      })
        .then(() => send('session/new', { cwd: deps.cwd ?? homedir(), mcpServers: [] }))
        .catch((error) => finish(undefined, error))
    })
  } finally {
    child.terminate()
  ...
  const thinkingOption = options.find((option) => option?.id === 'thinking' || option?.category === 'thought_level')
  const efforts = Array.isArray(thinkingOption?.options)
    ? thinkingOption.options
      .filter((option) => typeof option?.value === 'string' && option.value)
      .map((option) => ({ reasoningEffort: option.value, name: typeof option.name === 'string' && option.name ? option.name : option.value }))
    : []
  const defaultEffort = typeof thinkingOption?.currentValue === 'string' ? thinkingOption.currentValue : undefined
  const entries = Array.isArray(modelOption?.options) ? modelOption.options : []
  return entries
    .filter((entry) => typeof entry?.value === 'string' && entry.value)
    .map((entry) => ({
      provider: KIMI_OWN_PROVIDER,
      id: entry.value,
      name: typeof entry.name === 'string' && entry.name ? entry.name : entry.value,
...
export function createOwnConfigAdapter({ provider, name, readModel, modelList }) {
  ...
  return {
    ...
    async resolveModel(id, model) {
      ...
      return { provider: id, id: model, name: entry?.name ?? model, ...contextMetadata(entry) }
    },
...
export function createClaudeOwnConfigAdapter() {
  ...
  // 基类 resolveModel 查的是内部 list()(只有 cli-config/configured),
  // CLAUDE_MODELS 的 contextWindow 要在这里覆盖后按覆盖版 listModels 查。
  adapter.resolveModel = async (id, model) => {
    const entry = (await adapter.listModels()).find((item) => item.id === model)
    return { provider: id, id: model, name: entry?.name ?? model, ...contextMetadata(entry) }
  }
  return adapter
}
...
// Kimi:ACP configOptions 账号目录(模型 + thought_level 档位);失败回退
// 「跟随 CLI 配置」。选中具体模型时 adapter 用 session/set_config_option 下发。
export function createKimiOwnConfigAdapter(deps) {
  const rows = new Map()
  const adapter = createOwnConfigAdapter({
    provider: KIMI_OWN_PROVIDER,
    name: 'Kimi Code',
    modelList: deps ? () => kimiAccountModels(deps) : undefined,
  })
  const baseList = adapter.listModels.bind(adapter)
  adapter.listModels = async () => {
    const listed = await baseList()
...
export function ownConfigAdapters(deps) {
  return [
    ...
    createKimiOwnConfigAdapter(deps),
  ]
}
