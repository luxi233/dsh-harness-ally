...
// 优先用 deps.stateDir/native/kimi(持久);仅在没有 stateDir 时才回退到 tmpdir。
  ...
  // 会话级配置下发:所有路径都先把 mode 设成 auto(若支持);own-config
  // 模式下再按 DSH 选择下发 model / thinking(thought_level)档位。
  const applySessionConfig = async (configOptions) => {
    ...
    if (Array.isArray(modeOption?.options) && modeOption.options.some((option) => option?.value === 'auto')) {
