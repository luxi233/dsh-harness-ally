...
export async function startKimiAcpRun(deps, request) {
  ...
  try {
    ...
    // 启动 .cmd 会抛 spawn EINVAL。kimi.cmd shim 实际调 <basedir>/node_modules/@moonshot-ai/kimi-code/bin/kimi.js
    if (process.platform === 'win32' && typeof executable === 'string' && executable.toLowerCase().endsWith('.cmd')) {
      const basedir = dirname(executable)
      const jsEntry = join(basedir, 'node_modules', '@moonshot-ai', 'kimi-code', 'bin', 'kimi.js')
      argv = [process.execPath, jsEntry, 'acp']
    }
