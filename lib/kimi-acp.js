...
// 共享调试日志:写到 %TEMP%\dsh-ally-debug.log(失败静默,不影响主流程)
...
// 优先用 deps.stateDir/native/kimi(持久);仅在没有 stateDir 时才回退到 tmpdir。
    ...
    if (process.platform === 'win32' && typeof executable === 'string' && executable.toLowerCase().endsWith('.cmd')) {
      ...
      debugLog('kimiResolvedEntry', { executable, basedir, entry })
    }
    ...
    debugLog('kimiSpawn', {
      argv,
      cwd,
      envKeys: Object.keys(bridgeEnvironment(bridgeRoute, request, kimiHome)),
      kimiHome,
      bridgeRoutePresent: !!bridgeRoute,
    })
    ...
    debugLog('kimiSpawned', { pid: child.pid, platform: process.platform })
    let stderrBuf = ''
    child.stderr?.on('data', (chunk) => {
      stderrBuf += chunk.toString('utf8')
      if (stderrBuf.length > 4096) stderrBuf = stderrBuf.slice(-4096)
    })
    const exitWatcher = child.done?.then?.((outcome) => {
      debugLog('kimiExit', {
