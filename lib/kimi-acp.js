...
import { CLI_CONFIG_MODEL_ID, KIMI_OWN_PROVIDER } from './cli-own-models.js'
...
// 托管子进程的 done 可能早于最后一段 stdout 数据到达;退出判定前给在途数据一个短窗口。
const EXIT_STDOUT_GRACE_MS = 250
...
export async function startKimiAcpRun(deps, request) {
  ...
  let originalPrompt = request.nativeSession?.prompt ?? (textBlocks.length ? promptText(textBlocks) : '')
  if (typeof originalPrompt !== 'string' || (!originalPrompt.trim() && images.length === 0 && files.length === 0)) {
    throw new Error('Harness task must not be empty')
  }
  if (!originalPrompt.trim()) originalPrompt = images.length ? 'See the attached image(s).' : 'See the attached file(s).'
  const resolvedFiles = files.length ? await resolveFileInputs(deps, files) : []
  if (resolvedFiles.length) originalPrompt = filePathFallback(originalPrompt, resolvedFiles)
  ...
  // provider 'kimi-code' = 用 CLI 自有登录态:不开 bridge、不注 KIMI_MODEL_*,
  // 模型经 session/set_config_option 下发;其余 provider 走 DSH bridge 注入。
  const ownConfig = request.provider === KIMI_OWN_PROVIDER
...
// 优先用 deps.stateDir/native/kimi(持久);仅在没有 stateDir 时才回退到 tmpdir。
  ...
  // 会话级配置下发:所有路径都先把 mode 设成最高权限档(yolo 优先,退回
  // auto,与其它 Harness 的完全访问语义对齐);own-config 模式下再按 DSH
  // 选择下发 model / thinking(thought_level)档位。
  const KIMI_MODE_PREFERENCE = ['yolo', 'auto']
  const applySessionConfig = async (configOptions) => {
    ...
    const preferredMode = Array.isArray(modeOption?.options)
      ? KIMI_MODE_PREFERENCE.find((value) => modeOption.options.some((option) => option?.value === value))
      : undefined
    if (preferredMode) {
    ...
    if (request.model && request.model !== CLI_CONFIG_MODEL_ID && modelOption) {
      await sendRequest('session/set_config_option', { sessionId, configId: 'model', value: request.model })
    }
    const thinkingOption = options.find((option) => option?.id === 'thinking' || option?.category === 'thought_level')
    const thinkingValues = Array.isArray(thinkingOption?.options)
      ? thinkingOption.options.map((option) => option?.value)
      : []
    if (request.reasoningEffort && thinkingValues.includes(request.reasoningEffort)) {
      await sendRequest('session/set_config_option', { sessionId, configId: 'thinking', value: request.reasoningEffort })
    }
  }
  ...
  child.done.then((outcome) => {
    void (async () => {
      if (!settled && pending.size > 0) await new Promise((resolve) => setTimeout(resolve, EXIT_STDOUT_GRACE_MS))
      stdoutEnded = true
  ...
  void (async () => {
    try {
      ...
      phase = 'mode'
      await applySessionConfig(created.configOptions)
      ...
      if (recoverableSkillStop && state.skillRecoveryAttempts === 0 && !signal.aborted && !disposing) {
        ...
        if (recoveredFromTimeout && state.freshSkillRecoverySafe) {
          ...
          await applySessionConfig(recovered?.configOptions)
          if (signal.aborted || disposing || settled) throw new Error('Kimi Skill recovery aborted')
      ...
      if ((response?.stopReason === 'cancelled' && !state.skillContinuationTimedOut) || signal.aborted || disposing) {
        ...
        // kimi 把模型侧失败(如 bridge 上游 5xx/不支持的输入类型)吞掉后直接
        // end_turn,回合零输出零活动——按错误上报而不是静默 "completed"。
        if (!state.text && state.activitySnapshots.size === 0) {
          settle({ output, stopReason: 'error', diagnostic: 'Kimi Code 回合结束但未产生任何输出：模型调用可能失败（请确认所选模型支持当前输入类型）' }, true)
        } else {
      ...
      settle({
        output: state.text ? [{ type: 'text', text: state.text }] : [],
        stopReason: signal.aborted || disposing ? 'aborted' : 'error',
        ...(signal.aborted || disposing ? {} : { diagnostic: attachmentFailure ?? diagnostics[phase] ?? 'Kimi Code ACP 执行失败' }),
      }, true)
    }
  })()
