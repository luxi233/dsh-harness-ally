...
import { CLI_CONFIG_MODEL_ID, CODEX_OWN_PROVIDER } from './cli-own-models.js'
...
function appServerArgv(executable, bridge) {
  ...
  if (process.platform === 'win32' && typeof executable === 'string' && executable.toLowerCase().endsWith('.cmd')) {
    ...
    const jsEntry = join(nodes, '@openai', 'codex', 'bin', 'codex.js')
...
export async function startCodexAppServerRun(deps, request) {
  ...
  // turn/start 原生支持 localImage(宿主路径)与 image(data/远程 URL)输入项。
  ...
  // provider 'codex' = 用 CLI 自有配置:不开 bridge、不带 model_provider/
  // model 覆盖,子进程读用户真实的 ~/.codex/config.toml(provider、key、model)。
  const ownConfig = request.provider === CODEX_OWN_PROVIDER
  ...
  const capabilityModel = bridgeRoute ? CODEX_CAPABILITY_MODEL : ownConfig ? (request.model === CLI_CONFIG_MODEL_ID ? undefined : request.model) : request.model
    ...
    void (async () => {
      ...
      if (signal.aborted || disposing) {
        ...
        settle({
          ...
          stopReason: 'error',
          diagnostic: `codex app-server 提前退出（exit ${String(outcome.exitCode)}）${stderrTail.trim() ? `· stderr: ${stderrTail.trim().replaceAll(/\s+/g, ' ').slice(-300)}` : ''}`,
        }, false)
      }
    })()
  }, () => {
  ...
  void (async () => {
    try {
      ...
      const turn = await sendRequest('turn/start', {
        threadId,
        input: [
          ...resolvedImages.map((image) => (image.path
            ? { type: 'localImage', path: image.path }
            : { type: 'image', url: `data:${image.mediaType};base64,${image.data}` })),
