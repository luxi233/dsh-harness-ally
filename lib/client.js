window.__ModuleLoader__.load({
  ...
  const exports = module.exports
    ...
    const CSS = [
      ...
      ".ally-turn-badge{display:inline-flex;align-items:center;gap:4px;font-size:11px;line-height:18px;padding:1px 9px;border-radius:999px;border:1px dashed var(--dsw-alias-border-l2);color:var(--dsw-alias-label-secondary,var(--dsw-alias-label-primary))}",".ally-welcome-mount{display:contents}.uV2eYG_trailing{position:relative !important}.ally-welcome-mount .ally-engine{position:absolute !important;right:175px !important;top:50% !important;transform:translateY(-50%) !important;z-index:5;display:inline-flex !important}.ally-welcome-mount .ally-engine-trigger{width:auto !important;min-width:0 !important;white-space:nowrap !important;padding:0 10px !important}.ally-welcome-mount .ally-engine-copy{white-space:nowrap !important}.ally-welcome-mount .ally-engine-copy span{white-space:nowrap !important}",
    ].join("\n")

    function HarnessSelector({ sessionId, t, useSessions, useSession }) {
      const ally = useSessions((state) => {
        if (!sessionId) return true
        return state.current === sessionId && state.byId?.[sessionId]?.agentPreset === ALLY_PRESET
      })
    ...
    function apply(ctx) {
      ...
      // Welcome页兜底：conversation.input.right 在欢迎页不渲染( sessionId===void 0 )，直接在模型按钮左侧 DOM 注入
      ctx.effect(() => {
        const STORAGE_KEY = 'ally-welcome-harness'
        const getWelcomeHarness = () => {
          try { return localStorage.getItem(STORAGE_KEY) || 'dsh' } catch { return 'dsh' }
        }
        const setWelcomeHarness = (h) => {
          try { localStorage.setItem(STORAGE_KEY, h) } catch {}
          try { syncChannel?.postMessage({ type: 'selection', sessionId: '__welcome__' }) } catch {}
        }
        function WelcomeHarnessSelector() {
          const [harness, setHarness] = useState(getWelcomeHarness())
          const [open, setOpen] = useState(false)
          const rootRef = useRef(null)
          useEffect(() => {
            const h = () => setHarness(getWelcomeHarness())
          ...
          const choose = (h) => {
            if (h===harness) { setOpen(false); return }
            setHarness(h)
            setWelcomeHarness(h)
            setOpen(false)
          }
          // 完全复用原 HarnessSelector 的 DOM 结构与 CSS，仅数据来自 localStorage
          return h('div', { ref: rootRef, className: 'ally-engine' },
            h('button', { type: 'button', className: 'ally-engine-trigger', 'aria-label': 'Execution Harness', 'aria-expanded': open, onClick: () => setOpen(!open) },
              h('span', { className: 'ally-engine-mark' }, harnessIcon(harness, 'ally-engine-trigger-icon')),
              h('span', { className: 'ally-engine-copy' }, h('small', null, 'Harness'), h('span', null, LABELS[harness])),
              h('span', null, '▼')
            ),
            open ? h('div', { className: 'ally-engine-popover' },
              h('div', { className: 'ally-engine-head' }, h('div', { className: 'ally-engine-title' }, '选择Harness')),
              h('div', { className: 'ally-engine-options' },
                ...Object.keys(LABELS).map(harnessOpt => h('button', { key: harnessOpt, type: 'button', className: 'ally-engine-option', 'data-selected': harness===harnessOpt, onClick: () => choose(harnessOpt) },
                  harnessIcon(harnessOpt), h('span', { className: 'ally-engine-name' }, LABELS[harnessOpt]), h('span', { className: 'ally-engine-check' }, harness===harnessOpt ? '✓' : '')
                ))
              )
            ) : null
          )
        ...
        const mountWelcome = () => {
          // debug
          try { console.log('[ally-welcome] mountWelcome', { preset: document.querySelector('button.cubgiG_seat')?.textContent, modelBtn: !!document.querySelector('button[aria-label*="模型"]'), isAlly: document.querySelector('button.cubgiG_seat')?.textContent?.includes('Harness') }) } catch {}
          const presetBtn = document.querySelector('button.cubgiG_seat')
          const isAlly = presetBtn?.textContent?.includes('Harness') || presetBtn?.textContent?.includes('联盟')
          const modelBtn = document.querySelector('button[aria-label*="模型"]')
          if (!isAlly) {
            document.querySelectorAll('.ally-welcome-mount').forEach(el=>el.remove())
            return
          }
          if (!modelBtn) return
          let mount = document.querySelector('.ally-welcome-mount')
          if (!mount) {
            mount = document.createElement('div')
            mount.className = 'ally-welcome-mount'
            mount.style.display = 'contents'
            const parent = modelBtn.parentElement
            if (parent) parent.insertBefore(mount, modelBtn)
          }
          if (!mount.dataset.mounted) {
            ...
            try {
              ...
              if (createRoot) {
                ...
                root.render(h(WelcomeHarnessSelector, {}))
              } else if (ReactDOM.render) {
                ReactDOM.render(h(WelcomeHarnessSelector, {}), mount)
              }
            } catch {}
          }
        }
        const obs = new MutationObserver(mountWelcome)
        obs.observe(document.body, { childList: true, subtree: true })
        mountWelcome()
        return () => { obs.disconnect(); document.querySelectorAll('.ally-welcome-mount').forEach(el=>el.remove()) }
      }, 'ally.welcome-harness')
    }
