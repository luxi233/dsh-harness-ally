window.__ModuleLoader__.load({
  ...
  const exports = module.exports
    ...
    const CSS = [
      ...
      ".ally-turn-badge{display:inline-flex;align-items:center;gap:4px;font-size:11px;line-height:18px;padding:1px 9px;border-radius:999px;border:1px dashed var(--dsw-alias-border-l2);color:var(--dsw-alias-label-secondary,var(--dsw-alias-label-primary))}",".ally-welcome-mount{display:contents}.uV2eYG_trailing{display:flex !important;flex-wrap:nowrap !important;align-items:center !important;gap:12px !important}.ally-welcome-mount .ally-engine{display:inline-flex !important;flex:none !important}.ally-welcome-mount .ally-engine-trigger{width:auto !important;min-width:0 !important;white-space:nowrap !important;padding:0 10px !important}.ally-welcome-mount .ally-engine-copy{white-space:nowrap !important}.ally-welcome-mount .ally-engine-copy span{white-space:nowrap !important}",
    ].join("\n")
    ...
    function HarnessSelector({ sessionId, t, useSessions, useSession }) {
      ...
      if ((!ally && sessionId) || blank) return null
      ...
      ctx.effect(() => {
        ...
        function WelcomeHarnessSelector() {
          ...
          const choose = async (h) => {
            ...
            if (sessionId !== undefined && session?.blank && presetOfSession(session) === ALLY_PRESET) {
              try { await controller.select(sessionId, h) } catch {}
            }
          }
