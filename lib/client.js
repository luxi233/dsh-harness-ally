window.__ModuleLoader__.load({
  ...
  const exports = module.exports
    ...
    const LABELS = { dsh: 'DeepSeek Harness', 'claude-code': 'Claude Code', codex: 'Codex', 'kimi-code': 'Kimi Code', devin: 'Devin' }
    const ICONS = {
      dsh: {
        color: '#5786FE',
      ...
      devin: {
        ...
        viewBox: '0 0 128 128',
        paths: [
          'M6 93.2529V37.8398H10C10 40.8834 11.5856 43.6395 14.084 45.1855L14.3291 45.332L14.333 45.334L60.667 72.0713V116.347C60.6669 118.957 59.2702 121.374 57.0039 122.689C54.7312 124.009 51.9403 124.01 49.667 122.692L9.66699 99.5986L9.45215 99.4697C7.30992 98.1291 6 95.7813 6 93.2529ZM10 93.2529C10 94.4295 10.6306 95.5326 11.6709 96.1367L51.667 119.228L51.6709 119.23C52.7033 119.83 53.9637 119.83 54.9961 119.23C56.0363 118.626 56.6669 117.523 56.667 116.347V74.3818L12.334 48.7988C11.4848 48.3111 10.703 47.7295 10 47.0713V93.2529Z',
          'M56.667 11.6533C56.6669 10.4769 56.0363 9.37358 54.9961 8.76956C53.9638 8.17014 52.7032 8.17019 51.6709 8.76956L51.666 8.77248L14.333 30.3194C11.6446 31.8825 10.0002 34.7424 10 37.8399C10 40.8835 11.5854 43.6405 14.084 45.1865L14.3291 45.3321L14.333 45.335L56.667 69.7637V11.6533ZM60.667 76.6905L12.3379 48.8008C8.47075 46.5817 6 42.4312 6 37.8399C6.00017 33.3128 8.40716 29.1347 12.3291 26.8574L12.334 26.8545L49.6621 5.31155C51.9363 3.99105 54.7297 3.99021 57.0039 5.31057C59.2701 6.62645 60.6669 9.04345 60.667 11.6533V76.6905Z',
          'M67.3334 11.6533C67.3335 9.0435 68.7293 6.62647 70.9955 5.31057C73.2697 3.99008 76.063 3.99021 78.3373 5.31057L78.3363 5.31155L118.333 28.4014L118.337 28.4043C120.604 29.7203 122 32.1371 122 34.7471V90.1602H118C118 87.0185 116.31 84.1823 113.671 82.668L113.666 82.6651L67.3334 55.9014V11.6533ZM71.3334 53.5928L115.662 79.1992L116.021 79.4121C116.734 79.8534 117.396 80.3622 118 80.9278V34.7471C118 33.5706 117.37 32.4674 116.329 31.8633L76.3334 8.77248L76.3295 8.76956C75.2971 8.17014 74.0366 8.17019 73.0043 8.76956C71.964 9.37357 71.3335 10.4768 71.3334 11.6533V53.5928Z',
          'M118 90.1599C118 87.0182 116.31 84.1829 113.671 82.6686L113.666 82.6657L71.3334 58.2106V116.346C71.3334 117.523 71.964 118.626 73.0043 119.23C74.0366 119.83 75.2971 119.83 76.3295 119.23L76.3334 119.228L113.667 97.6813C116.361 96.1153 118 93.3052 118 90.1599ZM122 90.1599C122 94.7459 119.588 98.8687 115.671 101.143L115.666 101.145L78.3363 122.689L78.3373 122.69C76.0631 124.01 73.2696 124.011 70.9955 122.69C68.7292 121.374 67.3334 118.956 67.3334 116.346V51.2819L115.662 79.1989L116.021 79.4128C119.682 81.678 122 85.7121 122 90.1599Z',
        ],
      },
    }
    const PROVIDER_LABELS = { 'ally-claude-code': 'Claude Code', 'ally-codex': 'Codex', 'ally-kimi-code': 'Kimi Code', 'ally-devin': 'Devin' }
    ...
    function createController() {
      ...
      const current = (sessionId) => records.get(sessionId) ?? {
        harness: 'dsh',
        providers: { dsh: true, 'claude-code': false, codex: false, 'kimi-code': false, devin: false },
        dispatches: {},
        loading: false,
        ready: false,
        pending: false,
    ...
    function createCliController() {
      ...
      return {
        ...
        async load(sessionId) {
          ...
          // 已有快照时不在刷新期间降级 ready——否则安装中途/完成后的每次
          // 轮询都会让整列闪回"检查中"，已安装项也跟着消失。
    ...
    function harnessIcon(harness, className = 'ally-engine-icon') {
      ...
      }, (icon.paths ?? [icon.path]).map((d) => h('path', {
        d,
    ...
    const CSS = [
      ...
      // Note: 选择器只在 harness-ally 插件内定制，保持 DSH Web 外壳不变；见 .agents/notes/implemented/bug-fix/2026-09-11-harness-selector-spacing.md
      ".ally-engine-popover{position:absolute;right:0;bottom:calc(100% + 10px);z-index:50;box-sizing:border-box;width:336px;max-width:calc(100vw - 24px);padding:14px;border:1px solid var(--dsw-alias-border-l2);border-radius:16px;background:var(--dsw-alias-bg-module-platform);box-shadow:0 18px 48px rgba(0,0,0,.2)}",
      ".ally-engine-head{padding:2px 4px 14px}",
      ".ally-engine-title{font-size:13px;font-weight:600;line-height:18px;color:var(--dsw-alias-label-primary);letter-spacing:.1px}",
      ".ally-engine-options{display:grid;grid-template-columns:minmax(0,1fr);row-gap:8px}",
      ".ally-engine-option{box-sizing:border-box;display:grid;grid-template-columns:32px minmax(0,1fr) 22px;column-gap:12px;align-items:center;width:100%;height:56px;min-height:56px;margin:0;padding:0 12px;border:1px solid transparent;border-radius:12px;background:transparent;color:var(--dsw-alias-label-primary);font:inherit;text-align:left;cursor:pointer;overflow:visible;transition:background .12s ease,border-color .12s ease}",
      ".ally-engine-option:hover,.ally-engine-option[data-selected='true']{background:var(--dsw-alias-bg-hover,var(--dsw-alias-bg-base));border-color:var(--dsw-alias-border-l2)}",
      ".ally-engine-option:disabled{opacity:.52;cursor:not-allowed}",
      ".ally-engine-icon{width:24px;height:24px;display:block;justify-self:center;align-self:center;flex:none}",
      ".ally-engine-name{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:13px;font-weight:600;line-height:20px}",
      ".ally-engine-check{width:22px;height:22px;display:grid;place-items:center;justify-self:end;font-size:15px;line-height:1;color:var(--dsw-static-neutral-bluish-400)}",
      // 错误条单独隔一行,跟选项列表留出间距
      ".ally-engine-error{margin-top:4px;padding:8px 12px;border-radius:10px;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-error,#c22);font-size:11px;line-height:16px}",
      // missing/不可用行只禁用鼠标 + 把名字淡化,布局继承 .ally-engine-option
      ".ally-engine-option-missing{cursor:default}.ally-engine-option-missing .ally-engine-name{opacity:.65}",
      ".ally-engine-option-install{height:30px;padding:0 14px;border:1px solid var(--dsw-alias-border-l2);border-radius:8px;background:var(--dsw-alias-bg-module-platform);color:var(--dsw-alias-label-primary);font:inherit;font-size:11px;font-weight:500;cursor:pointer;transition:background .12s ease}.ally-engine-option-install:hover{background:var(--dsw-alias-bg-hover,var(--dsw-alias-bg-base))}.ally-engine-option-install:disabled{opacity:.55;cursor:not-allowed}",
    ...
    function HarnessSelector({ sessionId, t, useSessions, useSession }) {
      ...
      // running 进依赖:回合开始(派发)时服务端可能刚做了 provider→Harness
      // 自动切换,立即重拉 snapshot 让选择器图标同步翻转,而不是等回合结束。
      useEffect(() => { if (ally && sessionId) controller.load(sessionId) }, [ally, sessionId, running, completedTurns])
