...
export function createAllianceRuntime({ sessions, agents, gateway, state, isAgentLoopRequest }) {
  ...
  // 不往正在使用的 CLI 线程注入摘要指令。provider 有绑定 Harness 时优先
  // 进绑定的那个(codex 模型由 codex 摘要),不可用时回落到会话当前
  // Harness——压缩输入是自含全量文本,任何可用 CLI 都能完成。
  async function* routeCompaction(options, next) {
    ...
    const bound = OWN_PROVIDER_HARNESSES[options.provider]
    const current = state.harness(options.sessionId)
    let harness = bound ?? current
    if (harness !== current && !(await gateway.available(harness))) harness = current
    if (harness === 'dsh' || !(await gateway.available(harness)) || !agents.get(options.sessionId)) {
      yield* next()
      return
    }
    const conversation = createConversationView(options.messages ?? [])
