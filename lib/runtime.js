...
// own-config provider 与 Harness 一一绑定:选错 Harness 时必须在派发前
// 拒绝——否则外部 CLI 会一直重试打不通的 bridge 端点,回合静默空转。
...
function conversationBlockText(block, toolNames) {
  ...
  if (block.type === 'file') return '[file omitted from external Harness history]'
...
export function createAllianceRuntime({ sessions, agents, gateway, state, isAgentLoopRequest }) {
  ...
  async function* route(options, next) {
    ...
    if (requiredHarness && requiredHarness !== harness) {
      ...
      yield {
        type: 'finish',
        reason: { kind: 'error', failure: failure(`模型 provider「${options.provider}」只由 ${label} 执行：请把 Harness 从 ${current} 切换到 ${label}`, 'ALLY_HARNESS_MISMATCH') },
      }
      return
    }
