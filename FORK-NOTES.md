...
| `lib/runtime.js` (fork.11) | own-config provider under a mismatched Harness now auto-switches via `select()` instead of erroring — picking a `devin`/`codex`/etc provider model with the wrong Harness flips the session's Harness and dispatches to the bound CLI; `ALLY_HARNESS_MISMATCH` remains only when the switch itself fails (busy agent, unavailable CLI). The selector icon updates on the post-turn `controller.load` triggered by `completedTurns` |
