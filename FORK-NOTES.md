...
| `lib/client.js` (fork.11) | `controller.load` effect also depends on `running` — the selector icon re-syncs at dispatch time (right after the server-side auto-switch) instead of only at turn end via `completedTurns` |
