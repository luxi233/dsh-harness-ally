...
> 当前 fork 版本 `0.12.1-fork.8`,加了 **Windows + dsh-bridge customTunnel 远程访问 + Kimi Code 0.39.x + DSH 0.1.2 兼容**的补丁，并修正 Harness 选择器的间距与对齐。
...
让 DeepSeek Harness、Claude Code、Codex、Kimi Code 和 Devin 共享同一个 DSH 会话生命周期，并与 DSH 原生模型选择器自由组合。

- **模型**仍由 DSH 原生模型选择器决定，自动使用当前已配置的全部 provider/model。
- **Harness**由独立的「选择Harness」控件决定，每个回合可切换 DeepSeek Harness、Claude Code、Codex、Kimi Code 或 Devin。
...
- **真实外部执行**：选择 Claude Code、Codex、Kimi Code 或 Devin 后，由对应 CLI 实际执行；提示会明确区分外部执行 Harness 与 DSH 宿主。
- **用户可见内容实时输出**：Claude Code 使用 partial messages，Codex 使用 app-server，Kimi Code 与 Devin 使用 ACP `session/update`；正文不再等待进程结束后一次性出现。provider-private bridge reasoning、Kimi replay update 与敏感诊断仍会按安全边界过滤。
...
- **并行委派**：preset 同时提供 `subagent_claude_code`、`subagent_codex`、`subagent_kimi_code` 与 `subagent_devin`。
...
- **有条件的原生会话停泊与续接**：每个 DSH session 下的 Claude Code、Codex、Kimi Code 和 Devin lane 会各自停泊原生 session/thread；证明连续性后，连续使用只发送本轮请求，跨 Harness 切回只补交离开期间的已完成 canonical history。它是可丢弃的缓存优化而非事实源；证明失败、运行失败、配置变化或达到 32 个成功回合时自动安全 rollover。
...
- Claude Code / Codex / Kimi Code / Devin CLI 均为可选：可以预先全局安装，也可以在「选择Harness」菜单中按需安装（Devin 走官方 install.sh / setup.ps1 安装到用户目录，不经 npm）
- macOS、Linux、Windows
...
2. 打开「选择Harness」，选择 DeepSeek Harness、Claude Code、Codex、Kimi Code 或 Devin。
...
缺失的 Claude Code、Codex、Kimi Code 或 Devin 不可直接选择，只会显示「安装」按钮；安装完成后即可选择。CLI 解析顺序是“全局 `PATH` 优先，DSH 托管目录兜底”。

## 架构

| 平面 | 职责 |
| --- | --- |
| Host bundle | Harness selection、CLI 检测/托管安装、LLM waterfall router、本地模型协议 bridge、Claude partial/Codex app-server/Kimi ACP/Devin ACP adapters、有界 work ledger、DSH sandbox、provider registry |
...
- Devin 使用自己的 Cognition 模型后端，不经过 DSH 模型 bridge；`devin acp` 要求 host 侧 `authenticate`，凭据取自 `devin auth login` 落盘的 `credentials.toml`（或 `DEVIN_API_KEY` / `WINDSURF_API_KEY` 环境变量）；
...
- 状态文件 v3 最多保留 200 个原生 lane，并懒迁移 v2 记录；每个 session 的 dispatch 同时最多保留 400 个 turn，其中可带 v1 work ledger（20 个文件、10 条命令、10 条失败尝试）。旧 resume 记录仍可做一次连续恢复，成功后补齐水位线。Claude、Codex、Kimi 与 Devin 的持久化数据位于 DSH state 或 CLI 自有目录，不进入用户 CLI 的普通会话列表。vendor session 是可丢弃缓存，DSH Session 日志始终是唯一 canonical history。
...
- Kimi 与 Devin 的 ACP `tool_call*` 与其他外部工具活动都映射为 reasoning 中的只读状态行，不产生 `tool-call-delta`。
...
- Agent signal 会终止整个外部进程树；Codex 先尝试 `turn/interrupt`，Kimi Code 与 Devin 先发送 `session/cancel`。
...
- CLI 自有配置模式：模型选择器里还会出现 `Claude Code（CLI 配置）` 和 `Codex（CLI 配置）` provider。Claude 列出 `sonnet`/`opus`/`haiku` family alias 及 settings.json 里配置的模型；Codex 通过一次性 `codex app-server` `model/list` 握手拿账号级目录（含 reasoning efforts，失败时回退 config.toml 当前模型）。另有一个「CLI 配置」占位条目表示完全跟随 CLI 自己的配置。选中后对应 adapter 完全跳过 DSH bridge 和托管 config 目录，子进程用自己的 endpoint/凭据跑，模型名以 `--model`/thread model 显式下发；被不匹配的 Harness 使用会得到明确报错。Kimi 的 `KIMI_CODE_HOME` 已由插件托管，不提供此模式。
...
│   ├── devin-acp.js               # Devin ACP、host authenticate、session/load、cancel
